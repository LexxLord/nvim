------------------------------------------------------------------------------------------------------------------------
-- Version: 0.24.0
-- Author: Lexx_Lord <lexx-lord@mail.ru>
-- License: MIT
...
-- Запуск терминала в другом эмуляторе терминала =======================================================================
...
-- vim.fn.stdpath = function() return home_path end

-- vim.opt.packpath:append(home_path .. "site")
...
-- https://tproger.ru/news/neovim-0-12-0-poluchil-vstroennyj-menedzher-plaginov-vim-pack-i-ul

-- all_plugins = {}
-- for _, plugin in ipairs(vim.pack.get()) do
--     table.insert(all_plugins, plugin.spec.name)
--     -- print(plugin.spec.name)
-- end

-- vim.pack.del(all_plugins)       -- удалить установленные плагины
-- vim.pack.update(all_plugins)    -- обновить установленные плагины
-- vim.pack.update(nil)            -- обновить установленные плагины

-- https://tduyng.com/blog/vim-pack-and-snacks/
...
vim.pack.add({
    ...
    -- Neovim plugin to display command line output as a notification.
    -- Плагин Neovim для отображения выввода командной строки в виде уведомления.
    'https://github.com/folke/noice.nvim.git',
    -- Зависимости плагина Noice:
        -- UI Component Library for Neovim.
        -- Библиотека компонентов пользовательского интерфейса для Neovim.
        'https://github.com/MunifTanjim/nui.nvim.git',
        -- A Neovim plugin to display notifications.
        -- Плагин Neovim для отображения уведомлений.
...
w_key.add {
    ...
    -- Hotkeys/Commands help -------------------------------------------------------------------------------------------
    { '<leader>p', group = "Plugins", icon = { icon = "󰚥", color = 'orange' } },    -- 󱐥 󰚥   
    ...
    { '<leader>pu', icon = { icon = "", color = 'blue' } },
...
-- Удалить все неактивные плагины --------------------------------------------------------------------------------------
for _, plugin in ipairs(vim.pack.get()) do
    if not plugin.active then
        -- if vim.fn.input{prompt = 'Remove inactive plugin "'..plugin.spec.name..'"? (y/n): '}  == 'y'
        if vim.fn.input{prompt = 'Удалить неактивный плагин "'..plugin.spec.name..'"? (y/n): '}  == 'y'
        -- https://gist.github.com/casperlehmann/a8d0ba79ce0163ee36b1b7fb71861e2a
        -- https://dev.to/idanarye/moonicipal-explained-4h02
        -- https://github.com/nvim-telescope/telescope.nvim/issues/1479
        -- https://www.reddit.com/r/neovim/comments/17850is/how_do_i_respond_to_vimfninput_from_the_lua_api/
        then
            vim.pack.del({ plugin.spec.name })
            -- vim.notify("Удалено: "..plugin.spec.name)
            -- vim.notify("Deleted: "..plugin.spec.name)
        end
    end
end
-- all_plugins = {}
...
--         vim.notify("Невозможно удалить активный плагин: "..plugin.spec.name..
--         "\nЗакомментируйте плагин в файле конфигурации и повторите попытку.")
--         -- vim.notify("Cannot delete active plugin: "..plugin.spec.name..
--         -- "\nComment out the plugin in the configuration file and try again.")
--     else
--         vim.notify("Удалено: "..plugin.spec.name)
--         -- vim.notify("Deleted: "..plugin.spec.name)
--     end
-- end
-- all_plugins = table.concat(all_plugins, ", ")

-- print(all_plugins)
-- https://devforum.roblox.com/t/how-can-i-turn-a-table-to-a-string/3101545

-- vim.keymap.set("n", "<leader>pd", "<cmd>lua vim.pack.del({ "..all_plugins.." })<CR>", { desc = "Delete all plugins" })
...
vim.opt.cmdheight = 0               -- Высота командной строки cmdheight (0 - не показывать)
...
vim.keymap.set('n', '<leader><leader>', function()
    ...
    vim.notify("Config reloaded", "INFO", { title = "Nvim" })
...
-- Обновить плагины ----------------------------------------------------------------------------------------------------
vim.keymap.set("n", "<leader>pu", "<cmd>lua vim.pack.update()<CR>", { desc = "Update all plugins" })
...
-- КОНЕЦ ФАЙЛА И ПЕРЕЗАГРУЗКА КОНФИГА ==================================================================================
...
-- vim.notify("Info", "INFO", { title = "Nvim" })
-- vim.notify("Error", "ERROR", { title = "Nvim" })
-- vim.notify("Warn", "WARN", { title = "Nvim" })
-- vim.notify("Warn", vim.log.levels.WARN, { title = "Nvim" })
-- vim.notify("Debug", vim.log.levels.DEBUG, { title = "Nvim" })
-- vim.notify("Trace", vim.log.levels.TRACE, { title = "Nvim" })
